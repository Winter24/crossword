...
word_list = ['paladin', 'A heroic champion or paragon of chivalry.'], \
...
#test        
# word_list = ['Cat', 'Nya Nya'], ['Dog', 'Fckin Ugly'], ['Superman', 'Batman']
...
class CellEntry(Entry):
    # My ugly entry-that-allow-only-one-symbol-strings
...
class App:
    def __init__(self, crossword):
        global all
        #tạo cửa sổ game 100x100 cho to chút
        self.root = Tk()
        self.root.title("Group1_Assignment_CrossWord_Game")
        self._crossword = crossword

        # Set the background image
        self.img = ImageTk.PhotoImage(Image.open("./crossword_bg.jpg"))
        self.background_label = Label(self.root, image=self.img)
        self.background_label.place(x=0, y=0, relwidth=1, relheight=1)  # Make label fill the entire window

        self.start_gui()

    def start_gui(self):
        self._button_start_frame = Frame(self.root)
        self._button_start_frame.pack(side=BOTTOM, fill= BOTH, expand=True, pady=10)
        self._button_start_frame.configure(highlightthickness=0, background=self.root.cget('background'))
        Button(self._button_start_frame, text='Start', command=self.play_game).pack(pady=10)


    def play_game(self):
        self._button_start_frame.pack_forget()
        # Create a frame for the grid on top of the background
        self._grid = Frame(self.root)  # Set the background of the grid to transparent
        self._grid.pack(side=LEFT, fill=BOTH, padx=10, pady=10, expand=True)

        self._quiz_frame = Frame(self._grid)
        self._quiz_frame.pack(side=TOP, anchor='center',fill= BOTH, expand=True)

        self._button_frame = Frame(self._grid)
        self._button_frame.pack(side=BOTTOM, fill= BOTH, pady=10)
        
        self._text_frame = Frame(self.root, bg='white')  # Set background color and width as desired
        self._text_frame.pack(side=RIGHT, fill='both', padx=10, ipadx=30, pady=10)  # Pack the text frame to the right side of the window and fill in the vertical direction

        # Add text to the text frame
        self._text_label = Label(self._text_frame, text= a.legend() + '\n'*5, bg='white', justify=LEFT, wraplength=500)  # Set background color as desired
        self._text_label.pack(side= TOP, fill='x', padx=10, ipadx=10, pady=10)
        self._text_label.config(font=("Arial", 16))

        # tạo ô chữ (Cells) 
        self._cells = {}
        for col in range(1, self._crossword.cols + 1):
            self._quiz_frame.columnconfigure(col, weight=1, pad=20)  # Make columns expandable
            for row in range(self._crossword.rows):
                self._quiz_frame.rowconfigure(row, weight=1) # Make rows expandable
                c = self._crossword.get_cell(col, row)
                if c != '-':
                    entry = CellEntry(self._quiz_frame, width=4, justify=CENTER)
                    entry.grid(row=row, column=col, sticky="nsew")
                    self._cells[(col, row)] = entry
        
        # Đánh số cho cột + hàng (Numbers)
        for w in self._crossword.current_word_list:
            row, col = w.row, w.col
            if w.down_across() == 'down':
                row -= 1
            else:
                col -= 1
            Label(self._quiz_frame, text=str(w.number)).grid(column=col, row=row)

        #tạo nút để check
        Button(self._button_frame, text='Check', command=self.check).pack(pady=10)
      
    

    def check(self):
        # Iterate over each word in the crossword to check user input
        for w in self._crossword.current_word_list:
            col, row = w.col, w.row  # Starting position of the word
            for i in range(len(w.word)):
                # Determine the correct cell based on the orientation of the word
                if w.vertical:
                    current_row, current_col = row + i, col
                else:
                    current_row, current_col = row, col + i
                
                # Retrieve the entry widget for the current cell
                entry = self._cells.get((current_col, current_row))
                if entry:
                    # Compare the entry's value with the corresponding letter of the word
                    if entry.get().lower() != w.word[i]:
                        showwarning('...', f'Something went wrong start at collumn: {current_col}, row: {current_row}')
                        return
